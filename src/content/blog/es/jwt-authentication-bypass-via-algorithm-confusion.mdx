...
El token forjado conserva el mismo `kid` y cambia `alg` a HS256. Pasa el cursor por el header y el payload para ver el JSON decodificado:

<RequestDiff
  left="jwt-authentication-bypass-via-algorithm-confusion/12-original.txt"
  right="jwt-authentication-bypass-via-algorithm-confusion/12-jwt.txt"
