...
<Term id="concept.jwt">JWT</Term> <Term id="concept.algorithm-confusion">algorithm confusion</Term> (also known as key confusion) exploits servers that use **asymmetric** algorithms like RS256 but fail to enforce the expected algorithm when verifying tokens.
...
<RequestDiff
  left="jwt-authentication-bypass-via-algorithm-confusion/08-json.json"
  right="jwt-authentication-bypass-via-algorithm-confusion/08-hs256.json"
  leftLabel="Expected header"
  rightLabel="Confused header"
/>
