...
The <Term id="concept.dom">DOM</Term> (Document Object Model) is the browser's live representation of a page. When the browser receives HTML, a **parser** reads the tags and builds a **tree** of objects in memory: elements, attributes, and text nodes. That tree is the DOM.
...
Parsing is not a one-time event on first page load. Whenever code assigns an **HTML string** to a property like <Term id="concept.innerHTML">innerHTML</Term>, the browser runs the parser **again** on that string and creates new nodes from it.
