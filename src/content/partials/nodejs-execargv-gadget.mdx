...
After <Term id="concept.prototype-pollution">prototype pollution</Term>, `fork(script, args, options)` can start the child with attacker-controlled Node flags. `--eval` is the interesting one: the child runs arbitrary JavaScript during startup. From that JavaScript you can call `require("child_process").execSync(...)` and run OS commands.
