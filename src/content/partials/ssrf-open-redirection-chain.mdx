...
When an application can make arbitrary requests with <Term id="concept.ssrf">SSRF</Term> protections and also has an <Term id="concept.open-redirection">open redirection</Term> vulnerability, attackers can chain these to bypass the SSRF filters:
