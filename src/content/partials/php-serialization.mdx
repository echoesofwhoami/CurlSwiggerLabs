...
Serialization is the process of converting an object (a data structure in memory) into a format that can be stored or transmitted, like a string. <Term id="concept.php-deserialization">Deserialization</Term> is the reverse: turning that string back into an object. In PHP, this is done with `serialize()` and `unserialize()`.
