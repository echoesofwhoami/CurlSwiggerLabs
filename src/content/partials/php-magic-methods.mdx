...
PHP has special methods called **magic methods** that are automatically invoked at certain points in an object's lifecycle. They always start with a double underscore (`__`). The most relevant ones for <Term id="concept.php-deserialization">deserialization</Term> attacks are:
