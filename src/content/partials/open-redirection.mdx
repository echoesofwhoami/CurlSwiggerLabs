...
<Term id="concept.open-redirection">Open redirection</Term> is a vulnerability where an application accepts user-controlled input that determines where to redirect the user, without properly validating the destination URL. This allows attackers to redirect users to malicious websites.
...
- **Chain attacks** - combine with other vulnerabilities like <Term id="concept.ssrf">SSRF</Term>